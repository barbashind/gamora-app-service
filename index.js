...
app.post('/api/gamora/login', async (req, res) => {
  ...
  try {
      ...
      const token = jwt.sign({ username: user.username, role: user.role, companyId: user.companyId  }, 'secret-key-3', { expiresIn: '15h' });
      res.json({ token });
  } catch (error) {
      console.error(error);
...
app.get('/api/gamora/check-token', (req, res) => {
    ...
    jwt.verify(token, 'secret-key-3', (err) => {
        if (err) {
...
app.get('/api/gamora/get-user-info', async (req, res) => {
    ...
    jwt.verify(token, 'secret-key-3', async (err, decoded) => {
        if (err) {
        ...
        try {
            ...
            return res.json({ valid: true, username, role: user.role, companyId: user.companyId }); // Возвращаем статус и данные
        } catch (error) {
            console.error(error);
